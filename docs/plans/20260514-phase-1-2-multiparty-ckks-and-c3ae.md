...
- [ ] `partial_decrypt.go`: `PartialDecrypt(authenticatedCt) (multiparty.KeySwitchShare, error)`. Use `multiparty.NewKeySwitchProtocol(params, sigma_flood)` to instantiate the joint-decryption protocol with the flooding noise. Allocate share, call `proto.GenShare(c.skShare, zeroSk, authenticatedCt, &share)` (zero target sk = decryption). Return the share.
- [ ] tests:
...
- [ ] `keygen.go`: mirrors `internal/vclient/keygen.go`:
  - `RespondPKShare(sid, clientShare) → agentShare`: generate `pk_a` share, aggregate with `clientShare`, build `c.pkAgg` and `c.encryptor`. Return `pk_a` share so VClient can compute the same aggregate.
  - `RespondRLKRound1`, `RespondRLKRound2`: matching two-round flow, building `rlkAgg` after round 2.
  - `RespondGaloisShares(sid, clientShares) → (rlk, gks, err)`: validate `len(clientShares) == lambda-1`; for each index in ascending order combine with VAgent's matching share to produce a `*rlwe.GaloisKey`; assemble `*rlwe.GaloisKeySet`. Build `eval` from `rlkAgg + gksAgg`. Return `(rlkAgg, gksAgg)` so the caller (CLI) hands them to `vservice.StoreEvalKeys`.
...
- [ ] `finalize.go`: `FinalizeDecryption(sid, authenticatedCt, clientShare) → Verdict`. Compute VAgent's `KeySwitchShare` via the same `multiparty.NewKeySwitchProtocol(params, sigma_flood)` (note: VAgent uses `sigma_flood = 0` for its own share — only VClient floods; we still construct with `params.FloodSigma` for protocol symmetry and rely on the protocol to ignore it for VAgent's half — confirm against Lattigo's docs before coding). Aggregate shares, apply key-switch + decryption to recover the plaintext. Decode to slot vector. Call `a.auth.Ver(authKey, plaintext)`. If `!ok` → `VerdictReject`. Else `m > 0` → `Accept`, else `Reject`. Drop the session entry (single-use `authKey`).
- [ ] tests:
...
- [ ] propagate the change: VClient (`GenGaloisShares`) and VAgent (`RespondGaloisShares`) iterate `params.RotationIndices()` instead of `CanonicalRotationIndices(lambda)`. Update Task-5 and Task-6 tests accordingly.
