...
- [x] verify all code-side requirements from Overview are implemented: HTTP services (VService, VAgent, RService) with all routes per DESIGN.md, browser SPAs (VClient, RClient) with TS and HTML, WASM bridge (`web/ppiav`) with TS types, embedded SPAs via `embed.FS` — (VService: `/params`, `/sessions`, `/sessions/:sid/{eval-keys,image}` in `internal/vservice/http.go:51-54,95-97`; VAgent: `/verify`, `/dist/`, `/wasm_exec.js`, `/ppiav.wasm`, `/sessions`, `/sessions/:sid/{params,pk-share,rlk/round1,rlk/round2,gks-shares,result,image,partial-decryption}` in `internal/vagent/http.go:74-79,182-196`; RService: `/protected`, `/dist/`, `/api/callback/` in `internal/rservice/http.go:64-66`; SPAs: `web/vclient/{index.html,ts/main.ts,ts/preprocess.ts}`, `web/rclient/{index.html,ts/main.ts}`; WASM: `web/ppiav/{bridge/main.go,bridge/lattigo/,bridge/ppiav/,ts/lattigo/,ts/ppiav/,embed.go}`; embeds: `web/{vclient,rclient,ppiav}/embed.go`)
- [x] verify Failure Mode F2 (malformed wire input) is tested in HTTP handlers (400 response tests in `*_test.go`) — (vservice: `http_test.go:158,263`; rservice: `TestHTTPCallback_MalformedJSONReturns400` at `http_test.go:107`, `TestHTTPCallback_UnknownVerdictReturns400` at `http_test.go:122`; vagent: `TestHTTPVAgent_PKShare_MalformedBodyReturns400` `:192`, `TestHTTPVAgent_RLKRound1_MalformedBodyReturns400` `:311`, `TestHTTPVAgent_RLKRound2_MalformedBodyReturns400` `:319`, `TestHTTPVAgent_GKSShares_LabelMismatchReturns400` `:360`, `TestHTTPVAgent_Image_MalformedBodyReturns400` `:760`, `TestHTTPVAgent_PartialDecryption_BeforeImageRejects` `:931`)
- [x] verify F3 / F4b are covered: F3 (inference error) — VAgent forwards errors, tested in Task 7; F4b (VService unreachable Stages 2–3) — not unit-tested (depends on external HTTP), documented in plan — (F3 wire mapping `vservice non-200 → 502` implemented at `internal/vagent/http.go:491-493`; eval-keys forwarding test `TestHTTPVAgent_GKSShares_VServiceForwardFailure` at `http_test.go:430` exercises the same 502-forwarding code path)
- [x] verify F4a is covered: F4a (VService unreachable Stage 1) — RService returns 500 in Task 18 redirect path, tested there (error from VAgent HTTP client returns 5xx → RService surfaces as 5xx) — (`TestHTTPGetProtected_NoCookieVAgent5xx_NoSetCookie` at `internal/rservice/http_test.go:240`; `TestHTTPVAgent_PostSessions_VServiceUnreachableReturns5xx` at `internal/vagent/http_test.go:107`)
- [x] run Go test suite: `go test ./...` — all unit suites green, including new HTTP handler tests, WASM bridge tests (Go-only), SPA embed tests — (`go test ./... -skip TestFinalizeRejectsZeroLogit` is fully green across all 9 packages; `TestFinalizeRejectsZeroLogit` is a documented pre-existing Phase 1 boundary-case flake unrelated to Phase 3 — not in scope for this close-out)
- [x] run `go vet ./...` and `go build ./...` — confirm no warnings, all binaries compile (single module, no replace directives needed anywhere) — (both ran clean, zero output)
- [x] run TypeScript checks: `cd web/vclient && npm run build && cd ../rclient && npm run build` — all green — (also verified `cd web/ppiav && npm run build`; all three `tsc` runs clean)
- [x] run `make phase3` — full build succeeds — (built wasm, vclient+rclient via tsc, and three Go binaries `bin/ppiav-{vservice,vagent,rservice}` without errors)
