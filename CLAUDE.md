...
**`models.compile` requires `--reserve-output-levels 1`** for ppiav. The C3AE circuit consumes every modulus level by default, leaving `result_ct` at level 0; VAgent's MPD-Auth `MulNew` needs Level ≥ 1 and the mac step fails with `"resultCt at level 0 — Auth requires Level() >= 1"`. The flag (added in orion-v2-compiler 2.1.6) makes the bootstrap solver bump `input_level` by N and shift every node up so the final LT lands at level N — this is the only correct layer for the reservation (`ProtocolReserveLevels` in `internal/protocol/orion_params.go` is now a no-op set to 0; a Go-side chain extension would be silently eaten by the first lattigo LinearTransform). See commit `f7c6a6c` for the architecture rationale.
